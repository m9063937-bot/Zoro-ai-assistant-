...
export default function Visualizer({ state }: VisualizerProps) {
  ...
  // JARVIS color palette (Cyan/Blue) with Zoro's personality (Violet/Pink hints)
  const getTheme = () => {
    switch (state) {
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center overflow-hidden pointer-events-none">
      ...
      <motion.div
        ...
        className={`absolute w-[85%] h-[85%] rounded-full border-[2px] border-dotted ${theme.border} opacity-20 will-change-transform`}
      />

      {/* Ring 2: Scanner Ring */}
      <motion.div
        animate={getRingAnimation(1, false)}
        className={`absolute w-[65%] h-[65%] rounded-full border-[1px] ${theme.border} border-t-transparent border-b-transparent opacity-30 will-change-transform`}
      />

      {/* Ring 3: Core HUD Ring */}
      <motion.div
        animate={getRingAnimation(0, false)}
        className={`absolute w-[45%] h-[45%] rounded-full border-[2px] border-dotted ${theme.border} opacity-50 will-change-transform`}
      />
